...
def build_vertex_edge_graph_for_tdzdd(polyhedron_data: dict):
    """
    Build vertex-edge graph for TdZdd (.grh generation).
    
    Returns edges as a list of (v1, v2) tuples in edge_id order.
    
    TdZdd 用の頂点-辺グラフを構築（.grh 生成用）。
    
    edge_id 順の (v1, v2) タプルのリストとして辺を返します。
    
    Args:
        polyhedron_data (dict): Content of polyhedron_relabeled.json
    
    Returns:
        tuple: (num_vertices, edges) where edges is list of (v1, v2) tuples
    """
    edge_to_vertices = build_vertex_edge_graph(polyhedron_data)
    
    # Convert to list in edge_id order
    # edge_id 順のリストに変換
    edges = []
    for edge_id in sorted(edge_to_vertices.keys()):
        edges.append(edge_to_vertices[edge_id])
